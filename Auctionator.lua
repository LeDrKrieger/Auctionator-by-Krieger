...
function Atr_InitScanDB()
	...
	if (AUCTIONATOR_PRICE_EVOLUTION_DATABASE == nil) then
		AUCTIONATOR_PRICE_EVOLUTION_DATABASE = {};
	end
	if (AUCTIONATOR_PRICE_EVOLUTION_DATABASE[realm_Faction] == nil) then
		AUCTIONATOR_PRICE_EVOLUTION_DATABASE[realm_Faction] = {};
	end

	gAtr_ScanDB = AUCTIONATOR_PRICE_DATABASE[realm_Faction];
    gAtr_MeanDB = AUCTIONATOR_MEAN_PRICE_DATABASE[realm_Faction];
	gAtr_EvolutionDB = AUCTIONATOR_PRICE_EVOLUTION_DATABASE[realm_Faction];
...
function Atr_Init()
	...
	gEvolutionPane	= Atr_AddSellTab ("Evolution",		EVOLUTION_TAB);
...
local _AUCTIONATOR_EVOLUTION_TAB_INDEX = 0;
...
function Atr_FindTabIndex (whichTab)
	...
	if (_AUCTIONATOR_SELL_TAB_INDEX == 0) then
		...
		while (true)  do
			...
			if (tab.auctionatorTab) then
				...
				if (tab.auctionatorTab == EVOLUTION_TAB)	then _AUCTIONATOR_EVOLUTION_TAB_INDEX = i; end;
			end
	...
	if (whichTab == EVOLUTION_TAB)	then return _AUCTIONATOR_EVOLUTION_TAB_INDEX; end;
...
function Atr_AuctionFrameTab_OnClick (self, index, down)
	...
	elseif (Atr_IsAuctionatorTab(index)) then
		...
		if (index == Atr_FindTabIndex(EVOLUTION_TAB))	then gCurrentPane = gEvolutionPane; end;
		...
		if (index == Atr_FindTabIndex(EVOLUTION_TAB))	then AuctionatorTitle:SetText ("Auctionator - Evolution");			end;
		...
		if (index == Atr_FindTabIndex(EVOLUTION_TAB)) then
			Atr_Search_Box:Show();
			Atr_Search_Button:Show();
			Atr_EvolutionFrame:Show();
			Atr_ClearList();
			Atr_Evolution_ShowSearch (Atr_Search_Box:GetText());
		end
...
function Atr_IsModeEvolution ()
	return (Atr_IsTabSelected(EVOLUTION_TAB));
end
...
function Atr_AddMainPanel ()
	...
	Atr_CreateEvolutionFrame();
end

-----------------------------------------

function Atr_CreateEvolutionFrame()

	local frame = CreateFrame("FRAME", "Atr_EvolutionFrame", Atr_Main_Panel);
	frame:SetPoint ("TOPLEFT", 18, -82);
	frame:SetWidth (585);
	frame:SetHeight (300);
	frame:Hide();

	frame.title = frame:CreateFontString ("Atr_EvolutionTitle", "ARTWORK", "GameFontNormal");
	frame.title:SetPoint ("TOPLEFT", 0, 0);
	frame.title:SetText ("Price Evolution");

	frame.subtitle = frame:CreateFontString ("Atr_EvolutionSubtitle", "ARTWORK", "GameFontHighlightSmall");
	frame.subtitle:SetPoint ("TOPLEFT", 0, -24);
	frame.subtitle:SetText ("Enter an item name above to show prices saved by full scans.");

	frame.itemName = frame:CreateFontString ("Atr_EvolutionItemName", "ARTWORK", "GameFontNormalLarge");
	frame.itemName:SetPoint ("TOPLEFT", 0, -48);
	frame.itemName:SetText ("");

	frame.summary = {};

	local summaryLabels = { "Current", "14-day low", "14-day high", "Change" };

	for n = 1,4 do
		local box = CreateFrame ("FRAME", "Atr_EvolutionSummary"..n, frame);
		box:SetWidth (128);
		box:SetHeight (42);
		box:SetPoint ("TOPLEFT", (n - 1) * 138, -78);
		box:SetBackdrop ({
		...
		box.label = box:CreateFontString ("Atr_EvolutionSummaryLabel"..n, "ARTWORK", "GameFontDisableSmall");
		box.label:SetPoint ("TOPLEFT", 8, -7);
		box.label:SetText (summaryLabels[n]);

		box.value = box:CreateFontString ("Atr_EvolutionSummaryValue"..n, "ARTWORK", "GameFontHighlightSmall");
		box.value:SetPoint ("TOPLEFT", 8, -22);
		box.value:SetText ("-");

		frame.summary[n] = box;
	end

	frame.dayHeader = frame:CreateFontString ("Atr_EvolutionDayHeader", "ARTWORK", "GameFontDisableSmall");
	frame.dayHeader:SetPoint ("TOPLEFT", 4, -138);
	frame.dayHeader:SetText ("Day");

	frame.priceHeader = frame:CreateFontString ("Atr_EvolutionPriceHeader", "ARTWORK", "GameFontDisableSmall");
	frame.priceHeader:SetPoint ("TOPLEFT", 82, -138);
	frame.priceHeader:SetText ("Lowest price");

	frame.chartHeader = frame:CreateFontString ("Atr_EvolutionChartHeader", "ARTWORK", "GameFontDisableSmall");
	frame.chartHeader:SetPoint ("TOPLEFT", 176, -138);
	frame.chartHeader:SetText ("Trend");

	frame.dayHeader2 = frame:CreateFontString ("Atr_EvolutionDayHeader2", "ARTWORK", "GameFontDisableSmall");
	frame.dayHeader2:SetPoint ("TOPLEFT", 298, -138);
	frame.dayHeader2:SetText ("Day");

	frame.priceHeader2 = frame:CreateFontString ("Atr_EvolutionPriceHeader2", "ARTWORK", "GameFontDisableSmall");
	frame.priceHeader2:SetPoint ("TOPLEFT", 376, -138);
	frame.priceHeader2:SetText ("Lowest price");

	frame.chartHeader2 = frame:CreateFontString ("Atr_EvolutionChartHeader2", "ARTWORK", "GameFontDisableSmall");
	frame.chartHeader2:SetPoint ("TOPLEFT", 470, -138);
	frame.chartHeader2:SetText ("Trend");
	...
	for n = 1,14 do
		...
		local x = 4 + (column * 294);
		local y = -154 - ((rowIndex - 1) * 23);

		row.bg = frame:CreateTexture ("Atr_EvolutionRowBG"..n, "BACKGROUND");
		row.bg:SetPoint ("TOPLEFT", x - 2, y + 3);
		row.bg:SetWidth (274);
		row.bg:SetHeight (18);
		if ((rowIndex % 2) == 0) then
			row.bg:SetTexture (0.08, 0.08, 0.08, 0.35);
		else
			row.bg:SetTexture (0.14, 0.14, 0.14, 0.35);
		end

		row.day = frame:CreateFontString ("Atr_EvolutionDay"..n, "ARTWORK", "GameFontHighlightSmall");
		row.day:SetPoint ("TOPLEFT", x, y);
		row.day:SetText ("");

		row.price = frame:CreateFontString ("Atr_EvolutionPrice"..n, "ARTWORK", "GameFontHighlightSmall");
		row.price:SetPoint ("TOPLEFT", x + 78, y);
		row.price:SetText ("");

		row.track = frame:CreateTexture ("Atr_EvolutionTrack"..n, "BORDER");
		row.track:SetPoint ("TOPLEFT", x + 172, y + 3);
		row.track:SetWidth (96);
		row.track:SetHeight (10);
		row.track:SetTexture (0.16, 0.16, 0.16, 0.85);

		row.bar = frame:CreateTexture ("Atr_EvolutionBar"..n, "ARTWORK");
		row.bar:SetPoint ("LEFT", row.track, "LEFT", 0, 0);
		row.bar:SetWidth (1);
		row.bar:SetHeight (10);
		row.bar:SetTexture (1.0, 0.78, 0.15, 0.9);
...
function Atr_Evolution_ClearSummary()

	if (Atr_EvolutionFrame == nil) then
		return;
	end

	for n = 1,4 do
		Atr_EvolutionFrame.summary[n].value:SetText ("-");
	end
end

-----------------------------------------

function Atr_Evolution_SetRow (row, label, price, maxPrice)

	row.day:SetText (label);

	if (price and price > 0) then
		row.price:SetText (zc.priceToMoneyString (price, true));

		local barWidth = 1;
		if (maxPrice and maxPrice > 0) then
			barWidth = math.floor ((price / maxPrice) * 96);
			if (barWidth < 4) then
				barWidth = 4;
			end
		end
...
function Atr_Evolution_GetDisplayName (itemName)
	...
	if (gAtr_EvolutionDB == nil) then
		return itemName;
	end

	if (gAtr_EvolutionDB[itemName]) then
		return itemName;
	end

	local itemNameLC = string.lower (itemName);

	for name, data in pairs (gAtr_EvolutionDB) do
		if (string.lower (name) == itemNameLC) then
			return name;
		end
	end
...
function Atr_Evolution_ShowSearch (itemName)

	if (Atr_EvolutionFrame == nil) then
		return;
	end

	itemName = Atr_Evolution_GetDisplayName (itemName);

	if (itemName == "") then
		Atr_EvolutionFrame.itemName:SetText ("No item selected");
		Atr_EvolutionFrame.subtitle:SetText ("Enter an item name above to show prices saved by full scans.");
	else
		Atr_EvolutionFrame.itemName:SetText (itemName);
		Atr_EvolutionFrame.subtitle:SetText ("Last 14 days from Auctionator full scans.");
	end

	local itemData = gAtr_EvolutionDB and gAtr_EvolutionDB[itemName];
	local hasData = false;
	local maxPrice = 0;
	local minPrice = nil;
	local currentPrice = nil;
	local oldestPrice = nil;
	local prices = {};

	for n = 1,14 do
		local when = time() - ((n - 1) * 24 * 60 * 60);
		local key = date ("%Y-%m-%d", when);
		local price = itemData and itemData[key] or nil;
		...
		if (price and price > 0) then
			...
			if (minPrice == nil or price < minPrice) then
				minPrice = price;
			end
	...
	Atr_Evolution_ClearSummary();

	if (currentPrice) then
		Atr_EvolutionFrame.summary[1].value:SetText (zc.priceToMoneyString (currentPrice, true));
	end

	if (minPrice) then
		Atr_EvolutionFrame.summary[2].value:SetText (zc.priceToMoneyString (minPrice, true));
	end

	if (maxPrice and maxPrice > 0) then
		Atr_EvolutionFrame.summary[3].value:SetText (zc.priceToMoneyString (maxPrice, true));
	end

	if (currentPrice and oldestPrice and oldestPrice > 0 and currentPrice ~= oldestPrice) then
		local pct = math.floor (((currentPrice - oldestPrice) * 100 / oldestPrice) + 0.5);
		if (pct > 0) then
			Atr_EvolutionFrame.summary[4].value:SetText ("+"..pct.."%");
		else
			Atr_EvolutionFrame.summary[4].value:SetText (pct.."%");
		end
	end
	...
	for n = 1,14 do
		local when = time() - ((n - 1) * 24 * 60 * 60);
		local label = date ("%a %d/%m", when);

		Atr_Evolution_SetRow (Atr_EvolutionFrame.rows[n], label, prices[n], maxPrice);
	end

	if (itemName ~= "" and not hasData) then
		Atr_EvolutionFrame.subtitle:SetText ("No scan history for this item yet. Run full scans to build history.");
	end
end

-----------------------------------------

function Atr_Evolution_Search_Onclick()
...
function Atr_OnAuctionHouseClosed()
	...
	if (gAtr_FullScanState and gAtr_FullScanState ~= ATR_FS_NULL) then
		Atr_FullScanFail ("Scan cancelled", "Auction house closed.");
	end
...
function Atr_UpdateUI ()
	...
	if (Atr_IsTabSelected(EVOLUTION_TAB)) then
...
function Atr_IsTabSelected(whichTab)
	...
	if (not whichTab) then
		return (Atr_IsTabSelected(SELL_TAB) or Atr_IsTabSelected(MORE_TAB) or Atr_IsTabSelected(BUY_TAB) or Atr_IsTabSelected(EVOLUTION_TAB));
	end
...
function Atr_IsAuctionatorTab (tabIndex)
	...
	if (tabIndex == Atr_FindTabIndex(SELL_TAB) or tabIndex == Atr_FindTabIndex(MORE_TAB) or tabIndex == Atr_FindTabIndex(BUY_TAB) or tabIndex == Atr_FindTabIndex(EVOLUTION_TAB) ) then
