	...
	for name,scn in pairs (self.items) do
		...
		if (newprice > 0) then
			if (scn.itemQuality + 1 >= AUCTIONATOR_SCAN_MINLEVEL) then
				...
				Atr_RecordEvolutionPrice (scn.itemName, newprice);
			end
		end
	end
...
local ATR_FULLSCAN_FAST_TIMEOUT = 20;
local ATR_FULLSCAN_PAGE_TIMEOUT = 30;

local gAtr_FullScanMode;
local gAtr_FullScanWaiting;
local gAtr_FullScanQuerySent;
local gAtr_FullScanPage;
local gAtr_FullScanTotalAuctions;
local gAtr_FullScanAuctionsScanned;
...
function Atr_FullScanStart()
	...
	gAtr_FullScanIsSlowScan = false;
--	local gAtr_FullScanIsSlowScan = Atr_FullScan_Slow:GetChecked();
--	zc.md (gAtr_FullScanIsSlowScan);
	
	local canQuery,canQueryAll = CanSendAuctionQuery();
	
	if (canQueryAll or canQuery or gAtr_FullScanIsSlowScan) then
	
		Atr_FullScanSetStatus (ZT("Scanning").."...", "Waiting for auction house data.");
		...
		if (canQueryAll and not gAtr_FullScanIsSlowScan) then
			Atr_FullScanInitData();
			gAtr_FullScanMode			= "fast";
			gAtr_FullScanWaiting		= true;
			gAtr_FullScanQuerySent		= time();
			gAtr_FullScanPage			= 0;
			gAtr_FullScanAuctionsScanned = 0;

			Atr_FullScanSetStatus (ZT("Scanning").."...", "Requesting instant auction house scan.");
			QueryAuctionItems ("", nil, nil, 0, 0, 0, 0, 0, 0, true);
		else
			Atr_FullScanBeginPaged ("Instant scan is not available on this server; using page-by-page scan.");
		end
	else
		Atr_FullScanSetStatus ("Auction house is not ready yet.", "Try again in a few seconds.");
	end
...
function Atr_RecordEvolutionPrice (name, price)

	if (name == nil or name == "" or price == nil or price <= 0) then
		return;
	end
	...
	if (gAtr_EvolutionDB[name] == nil) then
		gAtr_EvolutionDB[name] = {};
	end

	local today = date ("%Y-%m-%d", time());
	local oldestToKeep = date ("%Y-%m-%d", time() - (30 * 24 * 60 * 60));

	gAtr_EvolutionDB[name][today] = price;

	for day, value in pairs (gAtr_EvolutionDB[name]) do
		if (day < oldestToKeep) then
			gAtr_EvolutionDB[name][day] = nil;
		end
	end
end
...
function Atr_FullScanSetStatus (status, detail)

	if (Atr_FullScanStatus) then
		Atr_FullScanStatus:SetText (status or "");
	end

	if (Atr_FullScanDetail) then
		Atr_FullScanDetail:SetText (detail or "");
	end
end

-----------------------------------------

function Atr_FullScanInitData()
...
function Atr_FullScanGetTotalPages()

	if (gAtr_FullScanTotalAuctions and gAtr_FullScanTotalAuctions > 0) then
		return math.ceil (gAtr_FullScanTotalAuctions / 50);
	end
...
function Atr_FullScanGetPageText()

	local totalPages = Atr_FullScanGetTotalPages();

	if (totalPages) then
		return (gAtr_FullScanPage + 1).."/"..totalPages;
	end

	return tostring (gAtr_FullScanPage + 1);
end

-----------------------------------------

function Atr_FullScanSetPagedStatus (detail)

	Atr_FullScanSetStatus (string.format ("Scanning page %s (%s)", Atr_FullScanGetPageText(), Atr_FullScan_GetDurString()),
		detail);
end

-----------------------------------------

function Atr_FullScanRecordAuction (index, lowprices, qualities)

	local name, texture, count, quality, canUse, level, minBid, minIncrement, buyoutPrice = GetAuctionItemInfo("list", index);

	if (name ~= nil and count ~= nil and count > 0 and buyoutPrice ~= nil) then

		if (gAtr_MeanDB[name] == nil) then
			gAtr_MeanDB[name] = {};
		end

		qualities[name] = quality or qualities[name] or 0;
...
function Atr_FullScanCommit (lowprices, qualities, numAuctionsScanned)

	gAtr_FullScanDur = time() - gAtr_FullScanStart;

	Atr_FullScanSetStatus (ZT("Processing"), "Updating Auctionator price database.");
	...
	for name,prices in pairs (lowprices) do
		...
		if (newprice > 0) then
			...
			local qx = (qualities[name] or 0) + 1;

			numEachQual[qx]	= (numEachQual[qx] or 0) + 1;
			...
			if (qx >= AUCTIONATOR_SCAN_MINLEVEL) then
				...
				Atr_RecordEvolutionPrice (name, newprice);
	...
	for name in pairs(gAtr_MeanDB) do
		if (type(gAtr_MeanDB[name]) == "table") then
			table.sort(gAtr_MeanDB[name]);
		end
	end

	gScanDetails.numBatchAuctions		= numAuctionsScanned or 0;
	...
	if (Atr_PrintBargains and Atr_CheckForBargain and GetNumAuctionItems("list") > 0) then
	...
	gAtr_FullScanWaiting = false;

	Atr_FullScanMoreDetails();

	Atr_FullScanSetStatus (ZT("Cleaning up"), "Finalizing scan results.");

	Atr_FullScanDone:Enable();

	Atr_FSR_scanned_count:SetText	(numAuctionsScanned or 0);
...
function Atr_FullScanBeginPaged (reason)

	Atr_FullScanInitData();

	gAtr_FullScanMode		= "paged";
	gAtr_FullScanWaiting	= false;
	gAtr_FullScanQuerySent	= nil;
	gAtr_FullScanPage		= 0;

	Atr_FullScanSetStatus (ZT("Scanning").."...", reason or "Using page-by-page scan.");
	Atr_FullScanQueryNextPage();
end

-----------------------------------------

function Atr_FullScanQueryNextPage()

	if (gAtr_FullScanState ~= ATR_FS_STARTED or gAtr_FullScanMode ~= "paged" or gAtr_FullScanWaiting) then
		return;
	end

	if (CanSendAuctionQuery()) then
		Atr_FullScanSetPagedStatus (string.format ("Requesting data. %d auctions processed so far.",
			gAtr_FullScanAuctionsScanned or 0));

		QueryAuctionItems ("", nil, nil, 0, 0, 0, gAtr_FullScanPage, nil, nil);
...
function Atr_FullScanProcessPage()
	...
	if (totalAuctions and totalAuctions > 0) then
		gAtr_FullScanTotalAuctions = totalAuctions;
	end

	for x = 1, numBatchAuctions do
		Atr_FullScanRecordAuction (x, gAtr_FullScanLowPrices, gAtr_FullScanQualities);
	end

	gAtr_FullScanAuctionsScanned = (gAtr_FullScanAuctionsScanned or 0) + numBatchAuctions;

	local done = (numBatchAuctions < 50);

	if (not done and gAtr_FullScanTotalAuctions and gAtr_FullScanTotalAuctions > 0) then
		done = ((gAtr_FullScanPage + 1) * 50 >= gAtr_FullScanTotalAuctions);
	end

	if (done) then
		Atr_FullScanCommit (gAtr_FullScanLowPrices, gAtr_FullScanQualities, gAtr_FullScanAuctionsScanned);
	else
		local scannedPageText = Atr_FullScanGetPageText();

		gAtr_FullScanPage = gAtr_FullScanPage + 1;

		Atr_FullScanSetPagedStatus (
			string.format ("Finished page %s. %d of %d auctions processed.",
				scannedPageText,
				gAtr_FullScanAuctionsScanned or 0,
				gAtr_FullScanTotalAuctions or 0));
	end
end

-----------------------------------------

function Atr_FullScanOnAuctionUpdate()

	if (gAtr_FullScanMode == "fast") then
		local numBatchAuctions, totalAuctions = GetNumAuctionItems("list");

		if (numBatchAuctions == 0 and totalAuctions == 0) then
			Atr_FullScanBeginPaged ("Instant scan returned no data; using page-by-page scan.");
			return;
		end

		if (totalAuctions and totalAuctions > 0 and numBatchAuctions < totalAuctions) then
			Atr_FullScanBeginPaged ("Instant scan returned partial data; using page-by-page scan.");
			return;
		end

		gAtr_FullScanWaiting = false;
		Atr_FullScanAnalyze();
	elseif (gAtr_FullScanMode == "paged" and gAtr_FullScanWaiting) then
		Atr_FullScanProcessPage();
	end
end

-----------------------------------------

function Atr_FullScanFail (status, detail)

	gAtr_FullScanState		= ATR_FS_NULL;
	gAtr_FullScanMode		= nil;
	gAtr_FullScanWaiting	= false;

	Atr_FullScanSetStatus (status, detail);
	Atr_FullScanDone:Enable();
...
function Atr_FullScanAnalyze()
	...
	Atr_FullScanSetStatus (ZT("Processing"), "Reading instant scan results.");
	...
	if (numBatchAuctions > 0) then
		...
		for x = 1, numBatchAuctions do
			...
			Atr_FullScanRecordAuction (x, lowprices, qualities);

			if (x % 100 == 0) then
				Atr_FullScanSetStatus (ZT("Processing").." ("..x..")", "Building lowest-price table.");
			end
		end
	end

	Atr_FullScanCommit (lowprices, qualities, numBatchAuctions);
end
...
function Atr_UpdateFullScanFrame()
	...
	if ((canQueryAll or canQuery) and gAtr_FullScanState == ATR_FS_NULL) then
		Atr_FullScanSetStatus ("", "");
		Atr_FullScanStartButton:Enable();
...
function Atr_FullScanFrameIdle()
	...
	if (gAtr_FullScanState == ATR_FS_STARTED) then
		...
		if (gAtr_FullScanMode == "fast") then
			if (gAtr_FullScanWaiting and gAtr_FullScanQuerySent and time() - gAtr_FullScanQuerySent > ATR_FULLSCAN_FAST_TIMEOUT) then
				Atr_FullScanBeginPaged ("Instant scan did not answer; using page-by-page scan.");
				return;
			end

			Atr_FullScanSetStatus (string.format ("Scanning (%s)", Atr_FullScan_GetDurString()),
				"Waiting for instant scan response.");

		elseif (gAtr_FullScanMode == "paged") then
			if (gAtr_FullScanWaiting) then
				if (gAtr_FullScanQuerySent and time() - gAtr_FullScanQuerySent > ATR_FULLSCAN_PAGE_TIMEOUT) then
					Atr_FullScanFail ("Auction house timed out",
						string.format ("No response for page %s after %d seconds.", Atr_FullScanGetPageText(), ATR_FULLSCAN_PAGE_TIMEOUT));
					return;
				end

				Atr_FullScanSetPagedStatus (string.format ("Waiting for auction house response. %d auctions processed so far.",
					gAtr_FullScanAuctionsScanned or 0));
			else
				Atr_FullScanQueryNextPage();

				if (not gAtr_FullScanWaiting) then
					Atr_FullScanSetPagedStatus (string.format ("Waiting for auction house cooldown. %d auctions processed so far.",
						gAtr_FullScanAuctionsScanned or 0));
				end
			end
		end
	...
	if (gAtr_FullScanState == ATR_FS_CLEANING_UP) then
		...
		Atr_FullScanSetStatus ("Cleaning up", "Clearing temporary auction list.");
		
		if (GetNumAuctionItems("list") < 100) then
			Atr_FullScanSetStatus (string.format ("Scan complete (%s)", Atr_FullScan_GetDurString()),
				string.format ("Added %d, updated %d.", gNumAdded or 0, gNumUpdated or 0));
